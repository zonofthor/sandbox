$(document).ready(function($) {
    ...
    $("#LeftColumn .Counter").attr(
        "title",
        "Klikk hér skrollar glugga niður að atriðum - eitt í einu"
    );
...
function selectStaff() {
    if ($("#Staff").prop("selectedIndex") > 0) {
        $("#divStaff .Counter").html(
            $("#Staff option:selected").attr("data-count")
        );
        $("#divStaff").addClass("Active");
        $(this).parent().find(".Counter").trigger("click");
    } else {
        $("#divStaff").removeClass("Active");
    }
}

function selectBlodAllt() {
    if ($(this).prop("selectedIndex") > 0) {
        $("#divBlodAllt .Counter").html(
            $("#BlodAllt option:selected").attr("data-count")
        );
        $("#divBlodAllt").addClass("Active");
        $(this).parent().find(".Counter").trigger("click");
    } else {
        $("#divBlodAllt").removeClass("Active");
    }
}
...
function selectExternal() {
    if ($(this).prop("selectedIndex") > 0) {
        ...
        $(this)
            .parent()
            .find(".Counter")
            .html($("#External option:selected").attr("data-count"));
        $(this).parent().find(".Counter").trigger("click");
    } else {
        $(this).parent().removeClass("Active");
    }
...
function clickPopupTrigger() {
    ...
    case "Blod1":
        ...
        $("#uiDialog>p").text(
                "Hér kemur svo nýtt og endurbætt skráningarviðmót síðar meir..."
            );
    ...
    case "mf_Lyfsedill":
        ...
        $("#uiDialog>p").text(
                "ATH bæði til að SKOÐA alla lyfseðla og SKRÁ NÝJA"
            );
    ...
    case "mf_Therapy":
        ...
        $("#uiDialog>p").text(
                "ATH bæði til að SKOÐA öll lyfjafyrirmæli og SKRÁ NÝ"
            );
    ...
    case "mf_Adgerdir":
        ...
        $("#uiDialog>p").text(
                "hægt að smella á aðgerð og stökkva inn í tímalínu til að skoða"
            );
    ...
    case "mf_Greiningar":
        ...
        $("#uiDialog>p").text(
                "hægt að smella á greiningu og stökkva inn í tímalínu til að skoða"
            );
        $("#uiDialog>img").attr("src", "Langtimagreiningar.jpg");
        break;
    case "mf_Vitals":
        $("#uiDialog>h1").text("Lífsmörk");
        $("#uiDialog>p").text("ATH bæði til að SKOÐA öll lífsmörk og SKRÁ");
...
// function dropUIDialog(ev) {
//     ev.preventDefault();
//     ev.stopPropagation();
//     // var left = $(ev.currentTarget)[0].offsetLeft;
//     // var top = $(ev.currentTarget)[0].offsetTop;
//     var left = $(this).position().left;
//     var top = $(this).position().top;
//     $("#uiDialog").css("left", left);
//     $("#uiDialog").css("top", top);
//     console.log(left);
//     console.log(top);
// }
